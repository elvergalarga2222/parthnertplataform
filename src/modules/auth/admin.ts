import { getCurrentPartner, type Partner } from "./service";
 ...
 * partner congelado nunca llega aquí (getCurrentPartner ya devuelve null).
 */
export async function getCurrentAdmin(): Promise<Partner | null> {
  const partner = await getCurrentPartner();
  if (!partner || !isAdminEmail(partner.email)) return null;
  return partner;
}
